...
BFMT::BFMT(const base::SpaceInformationPtr &si)
  ...
  , freeSpaceVolume_(si_->getStateSpace()->getMeasure())  // An upper bound on the free space volume is the total space
                                                          // volume; the free fraction is estimated in sampleFree
  , collisionChecks_(0)
  , nearestK_(true)
  , NNr_(0)
  , NNk_(0)
  ...
  , extendedFMT_(true)
{
  ...
  ompl::base::Planner::declareParam<unsigned int>("num_samples", this, &BFMT::setNumSamples, &BFMT::getNumSamples,
                                                  "10:10:1000000");
...
void BFMT::setup()
{
  if (pdef_)
  {
    ...
    nn_->setDistanceFunction(std::bind(&BFMT::distanceFunction, this, std::placeholders::_1, std::placeholders::_2));

    if (nearestK_ && !nn_->reportsSortedResults())
    {
      OMPL_WARN("%s: NearestNeighbors datastructure does not return sorted solutions. Nearest K strategy disabled.",
                getName().c_str());
      nearestK_ = false;
    }
  }
...
void BFMT::getPlannerData(base::PlannerData &data) const
{
  ...
  for (auto motion : motions)
  {
    ...
    if (inFwdTree)
    {
      ...
      {
        bool success = data.addEdge(base::PlannerDataVertex(motion->parent_[FWD]->getState(), fwd_tree_tag),
  ...
  for (auto motion : motions)
  {
    ...
    if (inRevTree)
    {
      ...
      {
        bool success = data.addEdge(base::PlannerDataVertex(motion->getState(), rev_tree_tag),
...
base::PlannerStatus BFMT::solve(const base::PlannerTerminationCondition &ptc)
{
  ...
  {
    NNk_ =
        std::ceil(std::pow(2.0 * radiusMultiplier_, (double)si_->getStateDimension()) *
                  (boost::math::constants::e<double>() / (double)si_->getStateDimension()) * log((double)nn_->size()));
